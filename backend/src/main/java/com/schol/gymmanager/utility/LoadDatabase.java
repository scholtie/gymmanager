...
class LoadDatabase {
    ...
    CommandLineRunner initDatabase(AddressRepository addressRepository, BusinessHoursRepository businessHoursRepository, CustomerRepository customerRepository, GeoRepository geoRepository, GymRepository gymRepository, PriceRepository priceRepository, SessionOptionRepository sessionOptionRepository, SessionRepository sessionRepository, SubscriptionPlanRepository subscriptionPlanRepository, SubscriptionRepository subscriptionRepository, TrainerRepository trainerRepository) {
        return args -> {
            ...
            Trainer trainer = trainerRepository.save(Trainer.builder().id(1L).email("trainer1@test.com").passwordHash("1234325fgerwg").firstName("Jane").lastName("Doe").gender(Gender.FEMALE).gym(gym).build());
            Trainer trainer2 = trainerRepository.save(Trainer.builder().id(2L).email("trainer2@test.com").passwordHash("1234325fgerwg").firstName("John").lastName("Smith").gender(Gender.MALE).gym(gym).build());
            Customer customer = customerRepository.save(Customer.builder().id(1L).role(Role.USER).email("customer1@test.com").passwordHash("1234325fgerwg").firstName("John").lastName("Doe").build());
