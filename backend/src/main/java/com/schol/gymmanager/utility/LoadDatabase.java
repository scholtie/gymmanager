...
class LoadDatabase {
    ...
    CommandLineRunner initDatabase(AddressRepository addressRepository,
        ...
        return args -> {
            ...
            SessionOption sessionOption = sessionOptionRepository.save(SessionOption.builder().id(1L).maxPeople(1).name("Deadlifting").lengthMinutes(60L).price(BigDecimal.valueOf(4000)).trainer(trainer).build());
            SessionOption sessionOption2 = sessionOptionRepository.save(SessionOption.builder().id(2L).maxPeople(10).name("Spinning").lengthMinutes(30L).price(BigDecimal.valueOf(2000)).trainer(trainer2).build());
            SessionOption sessionOption3 = sessionOptionRepository.save(SessionOption.builder().id(3L).maxPeople(5).name("Aerobics").lengthMinutes(30L).price(BigDecimal.valueOf(3000)).trainer(trainer).build());
